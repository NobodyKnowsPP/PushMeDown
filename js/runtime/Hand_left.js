...
export class Hand_left extends Sprite{
  constructor(){
    const image = Sprite.getImage('hand_left');
    super(image, 
      0, 0, image.width, image.height,
      window.innerWidth/3, window.innerHeight*2/5, 70, 160);
    ...
    if(place){//如果place为true则是要扭曲
      if(this.count > this.push_duration){
        super.draw(this.image, 
          ...
          window.innerWidth/7, window.innerHeight*2/5-this.push_duration, 
          70-this.push_duration, 160-this.push_duration*2);
      }else{
        super.draw(this.image, 
          0, 0, this.clippingWidth, this.clippingHeight, 
          window.innerWidth/7, window.innerHeight*2/5-this.count, 
      ...
      if(this.count > this.push_duration){
        super.draw(this.image,
          0, 0, this.clippingWidth, this.clippingHeight,
          window.innerWidth/3, window.innerHeight*2/5-this.push_duration, 
          70-this.push_duration, 160-this.push_duration*2);
      }else{
        super.draw(this.image, 
          0, 0, this.clippingWidth, this.clippingHeight, 
          window.innerWidth/3, window.innerHeight*2/5-this.count, 
          70-this.count, 160-this.count*2);
      }
